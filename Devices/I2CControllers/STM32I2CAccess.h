...
namespace LowLevelEmbedded
{
    ...
    public:
        ...
        bool I2C_ReadMethod(const uint8_t address, uint8_t* data, const size_t length) override
        {
            const HAL_StatusTypeDef st = HAL_I2C_Master_Receive(I2C_, address, data, static_cast<uint16_t>(length),
                                                          timeout_ms_);
            return (st == HAL_OK);
        }

        // Simple write: Master Transmit
        bool I2C_WriteMethod(const uint8_t address, uint8_t* data, const size_t length) override
        {
            const HAL_StatusTypeDef st = HAL_I2C_Master_Transmit(I2C_, address, data, static_cast<uint16_t>(length),
                                                           timeout_ms_);
            return (st == HAL_OK);
        }

        // Write then Read (common for register access without STOP in between if repeated start is used)
        bool I2C_ReadWriteMethod(const uint8_t address, uint8_t* data, const size_t readLength, const size_t writeLength) override
        {
        ...
        bool I2C_Mem_Read(const uint8_t address, const uint8_t memAddress, const uint8_t memAddSize, uint8_t* data,
                          const size_t length) override
        {
            const HAL_StatusTypeDef st = HAL_I2C_Mem_Read(I2C_, address, memAddress, memAddSize, data,
                                                    static_cast<uint16_t>(length), timeout_ms_);
            return (st == HAL_OK);
        }

        bool I2C_Mem_Write(const uint8_t address, const uint8_t memAddress, const uint8_t memAddSize, uint8_t* data,
                          const size_t length) override
        {
            const HAL_StatusTypeDef st = HAL_I2C_Mem_Write(I2C_, address, memAddress, memAddSize, data,
                                                    static_cast<uint16_t>(length), timeout_ms_);
            return (st == HAL_OK);
        }
        ...
        {
            const HAL_StatusTypeDef st = HAL_I2C_IsDeviceReady(I2C_, address, 3, timeout_ms_);
            return (st == HAL_OK);
        }

        [[nodiscard]] I2C_HandleTypeDef* Handle() const { return I2C_; }
        [[nodiscard]] uint32_t TimeoutMs() const { return timeout_ms_; }
